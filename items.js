...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        collection.aggregate(
            [{$project: {_id: 1, category: 1}}, {$group: {_id: "$category", num: {$sum: 1}}}],
            function(err, res){
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
        ...
        coll.find({$text: {$search: query, $caseSensitive: false}}).skip(page * itemsPerPage).limit(itemsPerPage).toArray(function(err, res){
            callback(res);
        });
    };
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        */
        var coll = this.db.collection("item");
        coll.find({$text: {$search: query, $caseSensitive: false}}).toArray(function(err, res){
            callback(res.length);
        });
    };
    ...
    this.getItem = function(itemId, callback) {
        ...
        coll.find({_id: itemId}).toArray(function(err, res){
            callback(res[0]);
        });
    };
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
        ...
        var coll = this.db.collection('item');
        coll.update({_id: itemId}, {$push: {reviews: reviewDoc}}, function(err, res){
            coll.find({_id: itemId}).toArray(function(error, response){
                callback(response[0]);
            });
        });
    };
